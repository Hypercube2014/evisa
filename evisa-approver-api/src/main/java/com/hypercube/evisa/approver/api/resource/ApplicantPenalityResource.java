...
@Data
@Slf4j
public class ApplicantPenalityResource{
    ...
    public ResponseEntity<VisaCheckOverstayDTO> estimatepenality(
    		 @RequestHeader(CommonsConstants.AUTHORIZATION) String authorization,
    		 @RequestBody VisaCheckOverstayDTO dto) {
        log.info("ApplicantVisaPenalityResource-estimatepenality");
         System.out.println("ApplicantVisaPenalityResource-searchVisaCheckOverstay");
         System.out.println(dto.getApplicationNumber()+dto.getLoggedUser());
        return new ResponseEntity<>(visacheckOverstayService.estimatePenality(dto),HttpStatus.OK);
    ...
    @CrossOrigin
    @PostMapping(value = "/v1/api/applypayementpenality")
    public ResponseEntity<PayementCash> applypayementpenality(
    		 @RequestHeader(CommonsConstants.AUTHORIZATION) String authorization,
    		 @RequestBody VisaCheckOverstayDTO dto) {
        log.info("ApplicantVisaPenalityResource-applypayementpenality");
         System.out.println("ApplicantVisaPenalityResource-searchVisaCheckOverstay");
         System.out.println(dto.getApplicationNumber()+dto.getLoggedUser());
       
        return new ResponseEntity<>(visaPenalityPayment.applypayements(dto),HttpStatus.OK);
    ...
    @CrossOrigin
    @PostMapping(value="/v1/api/statictiquecountBysecretaire")
    public ResponseEntity<PayementCash> statistiqueCount( 
    	 @RequestHeader(CommonsConstants.AUTHORIZATION) String authorization,
   		 @RequestBody VisaCheckOverstayDTO dto){


		return new ResponseEntity<>(visaPenalityPayment.statistiqueCount(dto.getLoggedUser(), dto.getDate1(), dto.getDate2()),HttpStatus.OK);
		
    }
    
    
    @CrossOrigin
    @PostMapping(value="/v1/api/statictiquesumBysecretaire")
    public ResponseEntity<PayementCash> statistiqueSum( 
    	 @RequestHeader(CommonsConstants.AUTHORIZATION) String authorization,
   		 @RequestBody VisaCheckOverstayDTO dto){
    	System.out.println("dto.getLoggedUser()+dto.getDate1()+dto.getDate2()"+dto.getLoggedUser()+dto.getDate1()+dto.getDate2());
		return new ResponseEntity<>(visaPenalityPayment.StatistiqueSum(dto.getLoggedUser(), dto.getDate1(), dto.getDate2()),HttpStatus.OK);
