...
@Component({
    selector: 'app-perfomance-reports',
    templateUrl: './perfomance-reports.component.html',
    styleUrls: ['./perfomance-reports.component.css']
})
export class PerfomanceReportsComponent extends BaseComponent implements OnInit {
    reportDetails: any = {}
    ...
    applyFilter(statisticalReportForm, reportDetails) {
        ...
        if (this.getToken('Role') === 'DMM') {
            if (!this.filterObj.oprType) {
                if (localStorage.getItem('Language') === 'en') {
                    this.toastr.errorToastr("Please Select Role Type  to Filter", 'Oops!');
                    return 0;
                } else {
                    this.toastr.errorToastr("Veuillez sélectionner le type de rôle à filtrer", 'Oops!');
                    return 0;
                }
            }
        }
    ...
    getFetchAgents(data) {
        ...
        this.commonService.callApi('performancereport/', data, 'post', false, false, 'APPR').then(success => {
            let successData: any = success;

            //console.log('merde ici', data);
            ...
            if (successData.content.length) {
                ...
                this.closed = this.perfomanceReports.map(function (el) { return el.closed; })
  ...
  ****************************************************************************/
    ...
    selectRole(e) {
        if (this.filterObj.reportType === 'I') {
            this.filterObj.agentList = '';
            this.getAgents(this.filterObj.oprType);
        }
    }
}
